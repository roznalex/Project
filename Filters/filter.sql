...
CREATE OR REPLACE TYPE g_filter_obj AS (
    ...
    MEMBER PROCEDURE get_all_users(from_in IN VARCHAR2(7), value_out OUT Varchar_tab)
)
...
CREATE OR REPLACE TYPE BODY g_filter_obj AS
    ...
    MEMBER PROCEDURE get_all_users(from_in IN VARCAHR2(7),
                                    value_out OUT Varchar_tab)
    BEGIN
        EXECUTE IMMEDIATE
        'SELECT DISTINCT'
        || last_modified_by
...
BEGIN
    ...
    ELSE
        ...
        IF f_filter_obj_in.g_filter_obj.user IS NOT NULL THEN
            v_temp := '(';
            FOR idx IN f_filter_obj_in.g_filter_obj.user.FIRST ..
                        (f_filter_obj_in.g_filter_obj.user.LAST - 1)
            LOOP
                v_temp := v_temp || f_filter_obj_in.g_filter_obj.user(idx) || ',';
            END LOOP;
            v_temp := v_temp || f_filter_obj_in.g_filter_obj.user(f_filter_obj_in.g_filter_obj.user.LAST) || ')';
...
CREATE OR REAPLACE PROCDURE f_filter ( p_filter_obj_in IN p_filter_obj,
                                        cur_out OUT SYS_REFCURSOR)
IS
...
BEGIN
    ...
    IF p_filter_obj_in.g_filter_obj.latestrecord = 1 THEN
        p_ilter_obj_in.g_filter_obj.latestrec(l_feature_piece);
        OPEN cur_out FOR l_feature_piece;
        CLOSE cur_out;
    ELSE
        ...
        IF p_filter_obj_in.g_filter_obj.user IS NOT NULL THEN
            v_temp := '(';
            FOR idx IN p_filter_obj_in.g_filter_obj.user.FIRST ..
                    (p_filter_obj_in.g_filter_obj.user.LAST - 1)
            LOOP
                v_temp := v_temp || p_filter_obj_in.g_filter_obj.user(idx) || ',';
            END LOOP;
            v_temp := v_temp || p_filter_obj_in.g_filter_obj.user(p_filter_obj_in.g_filter_obj.user.LAST) || ')';
